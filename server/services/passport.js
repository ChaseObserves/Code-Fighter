...
passport.serializeUser((user, done) => {
  // We are defining an anonymous arrow function and passing it to serializeUser, which is a Passport function
...
passport.use(
  new GoogleStrategy(
    ...
    async (accessToken, refreshToken, profile, done) => {
      // GoogleStrategy callback function querying Mongoose. Arguments here come from Passport
      ...
      if (existingUser) {
        // If a record does exist for the user's profile id, it will stop the user creation flow and log the user into its existing account."existingUser" represents the model instance of the user who was found in the search (if one was found aka if the user does have an account)
        ...
        // If no record exists for the person (id) attempting to sign in, create a new user instance for them
        const user = await new User({
          googleID: profile.id,
          name: profile.name.givenName
        }).save(); // This code creates a new mongoose model instance of a user. It represents a single record that might exist inside our database. This code then saves, or "persists" the user instance to the MongoDB database
